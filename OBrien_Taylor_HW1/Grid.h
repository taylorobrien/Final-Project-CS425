...
public:
	...
	void setContains(char containsChar);
	...
	GridNode* parent;		//used in A* to walk backwards
	int FValue;				//F value for the grid
	int GValue;				//G value for the grid
	int HValue;				//H value for the grid
};

class GridRow {  // helper class
public:
	std::vector<GridNode> data;		//data in the row (each col)
	GridRow(int size) {data.resize(size);};		//Initialized for the grid row
	~GridRow(){};
...
public:
	...
	void clearContains();				//clear what the gridnode has, ex. "b" or "c", etc. goes to "."
